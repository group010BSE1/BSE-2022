        ...
        if float(purchase_price) * 100 % 5 == 0:
            ...
            if checked_balance < 0:
                ...
                if int(money_to_be_returned) == 0:
                    ...
                    print("No change due\n")
                while count_1 < 3:
                    value_to_be_returned = money_to_be_returned//(values[2 - count_1] * 100)
                    if stock[2 - count_1] >= value_to_be_returned:
                        if int(value_to_be_returned) > 0:
                            print(int(value_to_be_returned), currency[count_1])
                            money_to_be_returned -= int(value_to_be_returned) * (values[2 - count_1]*100)
                            stock[2 - count_1] -= int(value_to_be_returned)
                        count_1 += 1
                    else:
                        if int(value_to_be_returned) > 0:
                            print(int(stock[2 - count_1]), currency[count_1])
                            money_to_be_returned -= stock[2 - count_1] * (values[2 - count_1]*100)
                            stock[2 - count_1] -= stock[2 - count_1]
                        count_1 += 1
